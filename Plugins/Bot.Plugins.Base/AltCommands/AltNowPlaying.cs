...
namespace Bot.Plugins.Base.AltCommands
{
    ...
    [AltCommandAttributes("Now Playing", true, new[] { "np", "nowplaying" })]
    public class AltNowPlaying : AltCommand
    {
        private ILog log = LogManager.GetLogger(typeof(AltNowPlaying));
        private IPersistentStore store;

        [ImportingConstructor]
        public AltNowPlaying([Import("Store")] IPersistentStore store)
        {
            this.store = store;
        }
